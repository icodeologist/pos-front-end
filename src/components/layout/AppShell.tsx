...
type IconName = "grid" | "cart" | "box" | "users" | "chart" | "receipt" | "settings" | "menu" | "close";
...
const links:{label:string;path:string;icon:IconName}[]=[
  {label:"Overview",path:"/",icon:"grid"},{label:"Point of Sale",path:"/billing",icon:"cart"},{label:"Products",path:"/products",icon:"box"},{label:"Customers",path:"/customer/register",icon:"users"},{label:"Reports",path:"/reports",icon:"chart"},{label:"Transactions",path:"/transactions",icon:"receipt"}
];
...
export default function AppShell() {
  ...
  return <div className="min-h-screen bg-slate-100">
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-slate-900 px-4 py-5 text-white transition-transform duration-300 lg:translate-x-0 ${open?"translate-x-0":"-translate-x-full"}`}>
      <div className="flex items-center justify-between px-2 pb-8"><button onClick={()=>{navigate("/");setOpen(false)}} className="flex items-center gap-3 text-left"><span className="grid h-10 w-10 place-items-center rounded-xl bg-orange-500 shadow-lg shadow-orange-950/30"><Icon name="receipt"/></span><span><span className="block text-lg font-extrabold tracking-tight">NexaPOS</span><span className="block text-[10px] font-bold uppercase tracking-[.2em] text-slate-400">Business Suite</span></span></button><button aria-label="Close menu" onClick={()=>setOpen(false)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-800 lg:hidden"><Icon name="close"/></button></div>
